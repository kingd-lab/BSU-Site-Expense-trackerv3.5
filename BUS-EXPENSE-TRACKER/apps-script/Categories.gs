/**
 * Categories.gs — SERVER-SIDE copy of js/categories.js's taxonomy and
 * guessCategory() logic, for use by bulk re-categorization functions in
 * Code.gs. Apps Script and the browser are separate runtimes, so this
 * can't simply import the client file — keep the two in sync manually
